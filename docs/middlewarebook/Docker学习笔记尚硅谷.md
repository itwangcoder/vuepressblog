...
2. 小总结
   ...
   ![](Docker学习笔记尚硅谷/基础篇/Docker镜像/Docker镜像commit操作案例小总结.png)
...
4. 将镜像推送到阿里云
   ...
   - 脚本实例
     ...
     ![](Docker学习笔记尚硅谷/基础篇/本地镜像发布到阿里云/将本地镜像推送到阿里云将镜像推送到阿里云脚本实例.png)
...
8. curl验证私服库上有什么镜像2
   ...
   ![](Docker学习笔记尚硅谷/基础篇/本地镜像发布到私有库/本地镜像发布到私有库将本地镜像推送到私有库第二次curl验证私服库有什么镜像.png)
...
5. 将一个redis.conf文件模板拷贝进 /opt/docker/redis 目录下
...
7. 使用redis6.0.8镜像创建容器(也叫运行镜像)
   ...
   ``` shell
   docker run -p 6379:6379 --name myredis6 --privileged=true -v /opt/docker/redis/redis.conf:/etc/redis/redis.conf -v /opt/docker/redis/data:/data -d redis:6.0.8 redis-server /etc/redis/redis.conf

8. 测试redis-cli连接上来

   ``` shell
   docker exec -it myredis6 /bin/bash
   redis-cli
   ```
...
9. 请证明docker启动使用了我们自己指定的配置文件
   ...
   - 修改后
     ...
     >
     > ```shell
     > docker restart myredis6

10. 测试redis-cli连接上来第2次

    ``` shell
    select 15  # 测试结果出现错误：(error) ERR DB index is out of range，说明现在数据库已经更改为10个
